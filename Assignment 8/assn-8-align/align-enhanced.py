...
def findOptimalAlignment(strand1, strand2,cache):
	...
	# they actually match.
	if cache.has_key(strand1[1:] + " " + strand2[1:]):
		bestWith = cache[strand1[1:] + " " + strand2[1:]]
	else:
	...
	if cache.has_key(strand1 + " " + strand2[1:]):
		bestWithout = cache[strand1 + " " + strand2[1:]]
	else:
	...
	# opposite scenario
	if cache.has_key(strand1[1:] + " " + strand2):
		bestWithout = cache[strand1[1:] + " " + strand2]
	else:
...
def main():
	while (True):
		...
		if answer == "no\n": break
		strand1 = generateRandomDNAStrand(60, 70)
		strand2 = generateRandomDNAStrand(60, 70)
