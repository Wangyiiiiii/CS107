 ...
 */
void doit(map<string,Definition> m,string begin);
...
int main(int argc, char *argv[])
{
 ...
 /* Production testpdt;
  ...
  testpdt=grammar[str].getRandomProduction();
  Production::iterator i1=testpdt.begin();
  cout<<*i1<<*(i1+1)<<endl;*/
...
{
   Production pdt;
   pdt = m[begin].getRandomProduction();
   Production::iterator i = pdt.begin();
   for (;i != pdt.end();i++){
     if ((*i)[0] != '<')
        cout << *i <<' ';
     else doit(m,*i); 
