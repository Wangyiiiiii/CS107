...
(define (intersection-points circles)
	(if (= (length circles) 1) '()
		(append (map (lambda (element) (intersect (car circles) element)) (cdr circles)) (intersection-points (cdr circles)) )))

;helperfirst to deal with the parens
(define (helperfirst points)
	(if (= (length points) 0) '()
		(append (car points) (helperfirst (cdr points))))) 

(define (distance-product point points)
	(apply * (map (lambda (element)
		(if (= (dist point element) 0) 1
			(dist point element))
				) points)))

(define (rate-points points)
	(map (lambda (element)
		(list (distance-product element points) element))
			points))
(define (sort-points points-rated)
	(quicksort points-rated))

(define (clumped-points points)
	 (map cadr (prefix-of-list (sort-points (rate-points points)) (quotient (length points) 2))))

(define (average-point points)
	(let ((ap (map (lambda (element) (/ element (length points))) (helpersecond (rate-points points)))))
		(list (car(car(rate-points(cons ap points)))) ap))) 


; (define (helperfirst point points)
; 	(if (= (length points) 0) 0
; 		(+ (dist point (car points)) (helperfirst point (cdr points)))))


;helpersecond to recursively get the sum of a series of points
(define (helpersecond points)
	(if (= (length points) 0) '(0 0)
		(add (cadr(car points)) (helpersecond(cdr points)) )))

(define (best-estimate circles)
	(average-point(clumped-points(helperfirst(intersection-points circles)))))

(define (where-am-i distances stars)
	(sort-points(map best-estimate (all-guesses distances stars))))

; (intersection-points'((1 (0 0)) (1 (2 0)) (0.1 (1 0))))
; (clumped-points '(((1.0 0.0) (1.0 0.0))
;  ((0.995 0.09987492177719066) (0.995 -0.09987492177719066))
;  ((1.005 -0.09987492177719066) (1.005 0.09987492177719066))))
; ;(load "where-am-i.scm")
; (helperfirst '((1.1 (1 0)) (2.2 (1 0)) (3.3 (1 0))))
; (helpersecond '((1.1 (1 0)) (2.2 (2 0)) (3.3 (3 0))))
; (cadr'((1.1 (1 0)) (2.2 (1 0)) (3.3 (1 0))))
; (car (car '((1.1 (0 0)) (2.2 (1 0)))))
;; Function: partition
; (helperfirst '(8/3 0) '((0 0) (2 0) (6 0)))
;; -------------------
...
;; (quicksort '((161.245 (3 2)) (481.663 (10 1)) (164.924 (2 5)) (320.224 (7 8))))
;; Function: quicksort
;; -------------------
